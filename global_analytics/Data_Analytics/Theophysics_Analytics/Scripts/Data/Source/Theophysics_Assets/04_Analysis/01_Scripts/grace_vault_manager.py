...
DEFAULT_DB = Path(__file__).resolve().parent.parent / "07_Data" / "coherence.db"
...
def cli():
    ...
    if args.auto:
        ...
        run_cmd([sys.executable, str(SCRIPTS / "utilities" / "vault_refresh.py"), "--vault", args.vault, "--db", str(db)])
        run_cmd([sys.executable, str(SCRIPTS / "analysis" / "cooccurrence_analyzer.py"), "--db", str(db)])
        run_cmd([sys.executable, str(SCRIPTS / "utilities" / "duplicate_finder.py"), "--db", str(db), "--threshold", str(args.threshold)])
        run_cmd([sys.executable, str(SCRIPTS / "utilities" / "auto_linker.py"), "--db", str(db), "--min-jaccard", str(args.min_jaccard)] + (["--markdown-links"] if args.markdown_links else []) + (["--write-yaml"] if args.write_yaml else []))
        run_cmd([sys.executable, str(SCRIPTS / "utilities" / "validation_scaffold.py"), "--db", str(db)])
        return
    ...
    if args.dupes:
        run_cmd([sys.executable, str(SCRIPTS / "utilities" / "duplicate_finder.py"), "--db", str(db), "--threshold", str(args.threshold)])
    if args.hubs:
        run_cmd([sys.executable, str(SCRIPTS / "utilities" / "concept_hub_generator.py"), "--db", str(db)])
    if args.validate:
        run_cmd([sys.executable, str(SCRIPTS / "utilities" / "validation_scaffold.py"), "--db", str(db)])
...
def gui():
    ...
    while True:
        ...
        if ev == "Scan & Index":
            ...
            run_cmd([sys.executable, str(SCRIPTS / "utilities" / "vault_refresh.py"), "--vault", vault, "--db", db])
            run_cmd([sys.executable, str(SCRIPTS / "analysis" / "cooccurrence_analyzer.py"), "--db", db])
        elif ev == "Co-Mentions":
            run_cmd([sys.executable, str(SCRIPTS / "analysis" / "cooccurrence_analyzer.py"), "--db", db])
        elif ev == "Find Duplicates":
            thr = float(vals["-DUPTHR-"]) / 100.0
            run_cmd([sys.executable, str(SCRIPTS / "utilities" / "duplicate_finder.py"), "--db", db, "--threshold", str(thr)])
        elif ev == "Run Auto-Linker":
            jthr = float(vals["-JACC-"]) / 100.0
            args = [sys.executable, str(SCRIPTS / "utilities" / "auto_linker.py"), "--db", db, "--min-jaccard", str(jthr)]
            if vals["-MDLINK-"]:
            ...
            run_cmd([sys.executable, str(SCRIPTS / "utilities" / "concept_hub_generator.py"), "--db", db])
        elif ev == "Run Validation (SIS/LCS/SRI)":
            run_cmd([sys.executable, str(SCRIPTS / "utilities" / "validation_scaffold.py"), "--db", db])
        elif ev == "Run All":
            if not vault: print("Select a vault folder first."); continue
            thr = float(vals["-DUPTHR-"]) / 100.0
            jthr = float(vals["-JACC-"]) / 100.0
