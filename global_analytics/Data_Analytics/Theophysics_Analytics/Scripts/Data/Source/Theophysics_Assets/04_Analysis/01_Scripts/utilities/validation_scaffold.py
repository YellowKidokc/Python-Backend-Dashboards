...
def compute(db_path: Path):
    ...
    # dashboard
    dashboard = db_path.parent.parent / "04_Dashboards"
...
def main():
    ...
    ap.add_argument("--db", default=str(Path(__file__).resolve().parents[2] / "07_Data" / "coherence.db"))
    args = ap.parse_args()
    n = compute(Path(args.db))
    print(f"Wrote metrics for {n} notes and generated 04_Dashboards/Validation_Dashboard.md")
